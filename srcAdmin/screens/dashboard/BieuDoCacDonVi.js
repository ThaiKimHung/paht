...
const DataChart = [
    { name: 'Đang xử lý (Trong hạn)', fill: '#FEA2B6' },
    { name: 'Đang xử lý (Quá hạn)', fill: '#8AC8F1' },
    { name: 'Đã xử lý (Trong hạn)', fill: '#FEE1A9' },
    { name: 'Đã xử lý (Quá hạn)', fill: '#ECEDEF' },
    { name: 'Tổng số PA', fill: '#97D9D9' }
]
const widthColumn = () => (nwidth() - 10) / 5
const BieuDoCacDonVi = (props) => {
    ...
    const getData = async (tungay = '', denngay = '') => {
        ...
        // }
        Utils.nlog("_______________-res1!!!,", res)
        if (res.status == 1) {
            setData(res.data)
        }
        else {
    ...
    const pieData = Data ?
        Data.map((value, index) => (
            [
                {
                    value: value.SLTrongHanDangXL,
                    TenDonVi: value.TenMuc,
                    svg: {
                        fill: '#FEA2B6',
                        onPress: () => Utils.getGlobal(nGlobalKeys.isChart, false, AppCodeConfig.APP_ADMIN) ? Utils.goscreen(props.nthis, ConfigScreenDH.DanhSachPhanAnhBD, { Value: value, Key: 5, tungay: dateFR, denngay: dateTO, LoaiMucDo: 5 }) : null,
                    },
                    key: `pie-0`,
                    // ...value
                },
                {
                    value: value.SLQuaHanDangXL,
                    TenDonVi: value.TenMuc,
                    svg: {
                        fill: '#8AC8F1',
                        onPress: () => Utils.getGlobal(nGlobalKeys.isChart, false, AppCodeConfig.APP_ADMIN) ? Utils.goscreen(props.nthis, ConfigScreenDH.DanhSachPhanAnhBD, { Value: value, Key: 5, tungay: dateFR, denngay: dateTO, LoaiMucDo: 6 }) : null,
                    },
                    key: `pie-1`,
                    // ...value
                },
                {
                    value: value.SLTrongHanDaXL,
                    TenDonVi: value.TenMuc,
                    svg: {
                        fill: '#FEE1A9',
                        onPress: () => Utils.getGlobal(nGlobalKeys.isChart, false, AppCodeConfig.APP_ADMIN) ? Utils.goscreen(props.nthis, ConfigScreenDH.DanhSachPhanAnhBD, { Value: value, Key: 5, tungay: dateFR, denngay: dateTO, LoaiMucDo: 2 }) : null,
                    },
                    key: `pie-2`,
                    // ...value
                },
                {
                    value: value.SLQuaHanDaXL,
                    TenDonVi: value.TenMuc,
                    svg: {
                        fill: '#ECEDEF',
                        onPress: () => Utils.getGlobal(nGlobalKeys.isChart, false, AppCodeConfig.APP_ADMIN) ? Utils.goscreen(props.nthis, ConfigScreenDH.DanhSachPhanAnhBD, { Value: value, Key: 5, tungay: dateFR, denngay: dateTO, LoaiMucDo: 3 }) : null,
                    },
                    key: `pie-3`,
                    // ...value
                },
                {
                    ...
                    svg: {
                        fill: '#97D9D9',
                        onPress: () => Utils.getGlobal(nGlobalKeys.isChart, false, AppCodeConfig.APP_ADMIN) ? Utils.goscreen(props.nthis, ConfigScreenDH.DanhSachPhanAnhBD, { Value: value, Key: 5, tungay: dateFR, denngay: dateTO, LoaiMucDo: 0 }) : null,
                    },
                    key: `pie-4`,
                    // ...value
                }
        ...
        <SafeAreaView style={{ flex: 1 }}>
            <View style={{}}>
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', }}>
                    ...
                    <Text style={{ fontWeight: 'bold', color: colors.redStar, paddingVertical: 10, fontSize: reText(18), width: widthColumn() * 4 + widthColumn() / 2, paddingLeft: 5 }} numberOfLines={2}>{'Các đơn vị xử lý'}</Text>
