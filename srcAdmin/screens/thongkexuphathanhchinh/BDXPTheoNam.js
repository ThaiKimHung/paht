...
const BDXPTheoNam = (props) => {
    ...
    const [selectLoaiDV, setselectLoaiDV] = useState('')
    ...
    const getData = async () => {
        let body = {
            ...objectFilter,
            Year: selectYear && selectYear.Nam ? selectYear.Nam : moment(new Date()).format('YYYY')
        }
        Utils.nlog("giá tị body", body)
        let res = await apis.ApiTKXPHC.BieuDo_XuPhatTheoThang(body);
        if (res.status == 1) {
            const pieData = res.data.map((item, index) => ({
                svg: {
                    fill: randomColor(),
                    onPress: () => Utils.goscreen(props.nthis, 'DanhSachCTChung', { Key: 3, thang: item.thang, nam: selectYear.Nam })
                },
                key: `pie-${index}`,
                ...item,
                value: item.SoLuong
