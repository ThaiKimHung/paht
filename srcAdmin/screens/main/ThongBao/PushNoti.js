...
import { Animated, View, Text, Image, Dimensions, Platform, StatusBar, StyleSheet, PanResponder, TouchableWithoutFeedback, TouchableOpacity } from 'react-native';
import { isIphoneX } from 'react-native-iphone-x-helper';
import { heightStatusBar, nwidth } from '../../../../styles/styles';
...
const getAnimatedContainerStyle = ({ containerSlideOffsetY, containerDragOffsetY, containerScale, typeKey }) => {
    ...
    const animatedContainerStyle = [
        typeKey == "popUpTinTuc" ? styles.popupContainerTinTuc : styles.popupContainer,
...
class PushNoti extends Component {
    ...
    constructor(props) {
        super(props);
        this.keygloblaVal = props.keygloblaVal;
        nthisPopUp[this.keygloblaVal] = this;
        this.state = {
            show: false,
    ...
    renderPopupContent = () => {
        ...
        return (
            <View style={[styles.popupContentContainer, this.keygloblaVal == "popUpTinTuc" ? { minHeight: 98 } : {}]}>
                ...
                {
                    this.keygloblaVal != "popUpTinTuc" ? null :
                        <TouchableOpacity activeOpacity={0.8} onPress={() => this.slideOutAndDismiss(300)}
                            style={{ alignItems: 'center', justifyContent: 'center', height: 36, width: '100%' }}>
                            <View style={{ width: 50, height: 4, borderRadius: 2, backgroundColor: colors.greyLight }} />
                        </TouchableOpacity>
                }
            </View>
        );
    ...
    render() {
        ...
        return (
            <Animated.View
                style={getAnimatedContainerStyle({ containerSlideOffsetY, containerDragOffsetY, containerScale, typeKey: this.keygloblaVal })}
...
const styles = StyleSheet.create({
    popupContainer: {
        position: 'absolute',
        width: nwidth() - (HORIZONTAL_MARGIN * 2) - 4,
        left: HORIZONTAL_MARGIN + 2,
        right: HORIZONTAL_MARGIN + 2,
        top: heightStatusBar() + 10 + 2
    },
    popupContainerTinTuc: {
        position: 'absolute',
        width: nwidth() - (HORIZONTAL_MARGIN * 2) + 4,
        left: HORIZONTAL_MARGIN - 2,
        right: HORIZONTAL_MARGIN - 2,
        top: heightStatusBar() + 10 - 3
    },
