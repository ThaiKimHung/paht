...
class TKTheoLinhVuc extends Component {
    constructor(props) {
        super(props)
        this.ChonDonVi_TKDV = Utils.getGlobal('ChonDonVi_TKDV', 'false', AppCodeConfig.APP_ADMIN)
        this.state = {
            ...
            TongSoPA: 0,
            ListDonVi: [],
            selectDonVi: { MaPX: -1, TenPhuongXa: "Tất cả" }
        }
    ...
    getListDonVi = async () => {
        let res = await apis.ApiQLThuMoi.GetListCQBH()
        Utils.nlog("Hoàng:", res)
        if (res && res.status == 1) {
            this.setState({ ListDonVi: [{ MaPX: -1, TenPhuongXa: "Tất cả" }, ...res.data] })
        }
        else {
    ...
    getData = async () => {
        const { FromDate, ToDate, selectDonVi } = this.state
        this.refLoading.current.show();
        let res = await GetList_ThongKePA_TheoLinhVuc(moment(FromDate, "DD/MM/YYYY").format("DD-MM-YYYY"), moment(ToDate, "DD/MM/YYYY").format("DD-MM-YYYY"), selectDonVi.MaPX == -1 || !selectDonVi.MaPX ? '' : selectDonVi.MaPX, 1);
        ...
        if (res.status == 1 && res && res.data && res.data.length > 0) {
            ...
            this.setState({ dataThongKe: [], TongSoPA: 0 })
        }
    ...
    onChangeTextIndex = (val, index) => {
        ...
        switch (index) {
            case 1:
                {
                    this.setState({ selectDonVi: val }, this.getData);
                }
                break;
            case 4:
                {
            ...
            case 1:
                {
                    Utils.goscreen(this.props.nthis, "Modal_ComponentSelectProps", {
                        ...
                        item: this.state.selectDonVi,
                        title: "Loại hiển thị",
                        AllThaoTac: this.state.ListDonVi,
    ...
    XemDanhSach = (key, IdDV, IdAccount, idDonVi) => {
        const { FromDate, ToDate } = this.state
        switch (key) {
            case KeyTK.SoLuong:
                Utils.goscreen(this.props.nthis, 'scChiTietTKAdmin', {
                    IdDonVi: IdDV, TuNgay: FromDate, DenNgay: ToDate, loaidanhgia: '0', Type: 4, IdAccount: IdAccount, idDonVi: idDonVi
                })
                break;
            case KeyTK.DaXL_Tong:
                Utils.goscreen(this.props.nthis, 'scChiTietTKAdmin', {
                    IdDonVi: IdDV, TuNgay: FromDate, DenNgay: ToDate, loaidanhgia: '1', Type: 4, IdAccount: IdAccount, idDonVi: idDonVi
                })
                break;
            case KeyTK.DaXL_TrongHan:
                Utils.goscreen(this.props.nthis, 'scChiTietTKAdmin', {
                    IdDonVi: IdDV, TuNgay: FromDate, DenNgay: ToDate, loaidanhgia: '2', Type: 4, IdAccount: IdAccount, idDonVi: idDonVi
                })
                break;
            case KeyTK.DaXL_QuaHan:
                Utils.goscreen(this.props.nthis, 'scChiTietTKAdmin', {
                    IdDonVi: IdDV, TuNgay: FromDate, DenNgay: ToDate, loaidanhgia: '3', Type: 4, IdAccount: IdAccount, idDonVi: idDonVi
                })
                break;
            case KeyTK.DangXL_Tong:
                Utils.goscreen(this.props.nthis, 'scChiTietTKAdmin', {
                    IdDonVi: IdDV, TuNgay: FromDate, DenNgay: ToDate, loaidanhgia: '4', Type: 4, IdAccount: IdAccount, idDonVi: idDonVi
                })
                break;
            case KeyTK.DangXL_TrongHan:
                Utils.goscreen(this.props.nthis, 'scChiTietTKAdmin', {
                    IdDonVi: IdDV, TuNgay: FromDate, DenNgay: ToDate, loaidanhgia: '5', Type: 4, IdAccount: IdAccount, idDonVi: idDonVi
                })
                break;
            case KeyTK.DangXL_QuaHan:
                Utils.goscreen(this.props.nthis, 'scChiTietTKAdmin', {
                    IdDonVi: IdDV, TuNgay: FromDate, DenNgay: ToDate, loaidanhgia: '6', Type: 4, IdAccount: IdAccount, idDonVi: idDonVi
                })
                break;
    ...
    render() {
        const { FromDate, ToDate, loaiHienThi, dataThongKe, ishowFilter, isChart, TongSoPA, selectDonVi } = this.state
        return (
            <View style={{ flex: 1 }}>
                    ...
                    <View style={{ flex: 1 }}>
                        ...
                        {!ishowFilter ? null :
                            ...
                            >
                                ...
                                {this.ChonDonVi_TKDV == 'true' ?
                                    <this.ComponentLinhVuc
                                        title={"Chọn đơn vị"}
                                        placeholder={"Chọn đơn vị"}
                                        value={selectDonVi.TenPhuongXa}
                                    ...
                                    dataThongKe.map((item, index) => {
                                        return (
                                            <View key={index} style={{ flexDirection: 'row', }}>
                                                ...
                                                <TouchableOpacity disabled={index == 0 ? true : false} onPress={() => this.XemDanhSach(KeyTK.SoLuong, item.IdMuc, item.IdAccount ? item.IdAccount : '', selectDonVi.MaPX == -1 || !selectDonVi.MaPX ? '' : selectDonVi.MaPX)}
                                                ...
                                                <TouchableOpacity disabled={index == 0 ? true : false} onPress={() => this.XemDanhSach(KeyTK.DaXL_Tong, item.IdMuc, item.IdAccount ? item.IdAccount : '', selectDonVi.MaPX == -1 || !selectDonVi.MaPX ? '' : selectDonVi.MaPX)}
                                                    style={{
                                                        ...styles.row,
                                                ...
                                                <TouchableOpacity disabled={index == 0 ? true : false} onPress={() => this.XemDanhSach(KeyTK.DaXL_TrongHan, item.IdMuc, item.IdAccount ? item.IdAccount : '', selectDonVi.MaPX == -1 || !selectDonVi.MaPX ? '' : selectDonVi.MaPX)}
                                                    style={{
                                                        ...styles.row,
                                                        borderWidth: 0.5, width: widthColumn() / 2,
                                                        borderRightWidth: 0,
                                                ...
                                                <TouchableOpacity disabled={index == 0 ? true : false} onPress={() => this.XemDanhSach(KeyTK.DaXL_QuaHan, item.IdMuc, item.IdAccount ? item.IdAccount : '', selectDonVi.MaPX == -1 || !selectDonVi.MaPX ? '' : selectDonVi.MaPX)}
                                                    style={{
                                                        ...styles.row,
                                                        borderWidth: 0.5, width: widthColumn() / 2,
                                                        borderRightWidth: 0,
                                                ...
                                                <TouchableOpacity disabled={index == 0 ? true : false} onPress={() => this.XemDanhSach(KeyTK.DangXL_Tong, item.IdMuc, item.IdAccount ? item.IdAccount : '', selectDonVi.MaPX == -1 || !selectDonVi.MaPX ? '' : selectDonVi.MaPX)}
                                                    style={{
                                                        ...styles.row,
                                                        borderWidth: 0.5, width: widthColumn() / 2,
                                                        borderRightWidth: 0,
                                                ...
                                                <TouchableOpacity disabled={index == 0 ? true : false} onPress={() => this.XemDanhSach(KeyTK.DangXL_TrongHan, item.IdMuc, item.IdAccount ? item.IdAccount : '', selectDonVi.MaPX == -1 || !selectDonVi.MaPX ? '' : selectDonVi.MaPX)}
                                                    style={{
                                                        ...styles.row,
                                                        borderWidth: 0.5, width: widthColumn() / 2,
                                                        borderRightWidth: 0,
                                                ...
                                                <TouchableOpacity disabled={index == 0 ? true : false} onPress={() => this.XemDanhSach(KeyTK.DangXL_QuaHan, item.IdMuc, item.IdAccount ? item.IdAccount : '', selectDonVi.MaPX == -1 || !selectDonVi.MaPX ? '' : selectDonVi.MaPX)}
                                                    style={{
                                                        ...styles.row,
                                                        borderWidth: 0.5, width: widthColumn() / 2,
                                                        borderRightWidth: 0,
