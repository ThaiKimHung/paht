...
export class DetailsCBCV extends Component {
    constructor(props) {
        super(props)
        this.callback = Utils.ngetParam(this, "callback", () => {
            ROOTGlobal.dataGlobal._reloadSOS()
            Linking.openURL(appConfig.deeplinkHomeCBCV)
        });
    ...
    GetDetailsSOS = async () => {
        nthisIsLoading.show();
        let res = await apis.ApiCBCV.Info_SOS(this.ID)
    ...
    GetList_NhatKyThaoTac = async () => {
        nthisIsLoading.show();
        let res = await apis.ApiCBCV.GetList_NhatKyThaoTac(this.ID)
    ...
    GetListFileThaoTac = async () => {
        nthisIsLoading.show();
        let res = await apis.ApiCBCV.Info_SOSDetail(this.ID)
    ...
    ThuHoiSOS = () => {
        Utils.showMsgBoxYesNo(this, 'Thông báo', 'Bạn có chắc muốn thu hồi tin cảnh báo covid này không ?', 'Thu hồi', 'Hủy',
            async () => {
                nthisIsLoading.show();
                let res = await apis.ApiCBCV.ThuHoiSOS(this.state.DataDetails)
                Utils.nlog('res thu hoi sos', res)
                if (res.status == 1) {
                    nthisIsLoading.hide();
                    Utils.showMsgBoxOK(this, 'Thông báo', 'Thu hồi tin cảnh báo covid thành công', 'Xác nhận', () => {
                        this.callback()
                        ROOTGlobal.dataGlobal._reloadSOS(1, this.ID)
                    })
                } else {
                    nthisIsLoading.hide();
                    Utils.showMsgBoxOK(this, 'Thông báo', 'Thu hồi tin cảnh báo covid thất bại', 'Xác nhận')
                }
            })
    }

    XoaSOS = () => {
        Utils.showMsgBoxYesNo(this, 'Thông báo', 'Bạn có chắc muốn xóa tin cảnh báo covid này không ?', 'Xóa', 'Hủy',
            async () => {
                nthisIsLoading.show();
                let res = await apis.ApiCBCV.DeleteSOS(this.ID)
                Utils.nlog('res xóa sos', res)
                if (res.status == 1) {
                    nthisIsLoading.hide();
                    Utils.showMsgBoxOK(this, 'Thông báo', 'Xóa tin cảnh báo covid thành công', 'Xác nhận', () => {
                        this.callback()
                        ROOTGlobal.dataGlobal._reloadSOS(1, this.ID)
                    })
                } else {
                    nthisIsLoading.hide();
                    Utils.showMsgBoxOK(this, 'Thông báo', 'Xóa tin cảnh báo covid thất bại', 'Xác nhận')
                }
            })
    }
    ...
    handleButton = (item) => {
        ...
        switch (item.Key) {
            case KeyButton.ChuyenXuLy:
                ...
                Utils.goscreen(this, 'Modal_ChuyenSuaCBCV', {
            ...
            case KeyButton.XuLy:
                ...
                Utils.goscreen(this, 'Modal_XuLyCBCV', { item: { ...this.state.DataDetails }, title: item.Name, buttonHandle: item, callback: this.callback })
                break;
    ...
    render() {
        ...
        return (
            <View style={{ flex: 1, backgroundColor: colors.BackgroundHome }}>
                {/* Header */}
                <HeaderCom
                    titleText={'Chi tiết cảnh báo covid'}
...
export default Utils.connectRedux(DetailsCBCV, mapStateToProps, true);
