...
class HomeCBCV extends Component {
    constructor(props) {
        super(props);
        this.state = {
            dataTinhTrang: [],
    ...
    _GetListSOS = async () => {
        ...
        let obj = {
            ...objFilter,
            "keyword": keyword,
            "filter.vals": `${dateFrom ? moment(dateTo, 'YYYY-MM-DD').format('DD-MM-YYYY') : ''}|${dateFrom ? moment(dateFrom, 'YYYY-MM-DD').format('DD-MM-YYYY') : ''}|${tinhtrang.Id}|1`,
            "page": page.Page,
        }
        let res = await apis.ApiCBCV.GetListSOS(obj)
    ...
    _goDetail = (item) => {
        Utils.goscreen(this, 'scDetailsCBCV', { ID: item.Id, callback: () => Utils.goscreen(this, 'scHomeCBCV') })
    }
