...
const DanhSachChotCho = (props) => {
    const { listProvine = [], ChotCho = {} } = useSelector(state => state.datahcm);
    ...
    useEffect(() => {
        if (ChotCho?.IdTram) {
            Utils.navigate('Modal_QuetMaChotCho', { isSaveChotKiem: true })
        }
        return () => {
    ...
    const getListChot = async (isNext = false) => {
        ...
        if (Quan && Phuong) { // cũ là (Tinh && Quan)
            ...
            let res = await apis.ApiChotKiem.GetListDSChotKiem(isNext ? Page.Page + 1 : 1, 10, Tinh?.IdDonVi ? Tinh.IdDonVi : '', Quan?.IdDonVi ? Quan.IdDonVi : '', Phuong?.IdDonVi ? Phuong.IdDonVi : '', 1)
    ...
    const ChonChot = (item) => {
        ...
        dispatch(ActionHCM.SaveChotCho(item))
        Utils.navigate('Modal_QuetMaChotCho', { isSaveChotKiem: true })
    }
            ...
            case TYPES.DropDown:
                if (item.isRow) {
                    return useMemo(() => <ComponentItem.ComponentDrop  {...item} isDrop={true} key={index} value={objectData[key] ? objectData[key][keyView].toUpperCase() : ''}
                        ...
                        styleContainer={isEnd ? stDanhSachChotCho.dropdownContainerIsEnd : stDanhSachChotCho.dropdownContainer}
                        styleContentLabel={{ marginTop: -10 }}
                        styleBodyInput={stDanhSachChotCho.compBodyInput}
    ...
    const RenderItem = ({ item, index }) => {
        return (
            <TouchableOpacity key={index} onPress={() => ChonChot(item)}>
                <View style={stDanhSachChotCho.itemList}>
                    <Image source={Images.icChotKiemDich} style={nstyles.nIcon20} resizeMode='contain' />
                    <View style={{ flex: 1, paddingLeft: 15 }}>
                        <Text style={stDanhSachChotCho.txtChotKiem} numberOfLines={2}>{item.TenTram}</Text>
                        <Text style={stDanhSachChotCho.txtDiaChi}>
    ...
    return (
        <View style={{ flex: 1, backgroundColor: colors.white }}>
            <HeaderCus
                ...
                title={`CHỐT QUÉT PHIẾU ĐI CHỢ`}
                styleTitle={{ color: colors.white, fontSize: reText(20) }}
            />
            <View style={stDanhSachChotCho.Container}>
                <View style={stDanhSachChotCho.HeaderList}>
                    {
                ...
                <FlatList
                    ...
                    ItemSeparatorComponent={() => {
                        return <View style={stDanhSachChotCho.ItemSeparatorComponent} />
                    }}
...
const stDanhSachChotCho = StyleSheet.create({
    Container: {
        flex: 1,
...
export default DanhSachChotCho
