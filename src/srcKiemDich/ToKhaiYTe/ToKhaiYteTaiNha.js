...
const ToKhaiYTeTaiNha = (props) => {
    ...
    const onPressSubmid = async (checkFaceAI = false, base64Img = "") => {
        ...
        //---Check Xác thực khuôn mặt trước khi Gửi Khai Báo
        if (checkFaceAI != true) {
            Utils.goscreen({ props }, 'Modal_Camera', { isMode: 1, callback: (base64) => onPressSubmid(true, base64) });
            return;
        }
        //---
        ...
        Utils.nlog("body", JSON.stringify(body))
            ...
            <View style={customStyles.containerBody}>
                <KeyboardAwareScrollView style={{ backgroundColor: colors.white }} showsVerticalScrollIndicator={false}>
                    <View style={{ paddingVertical: FontSize.reText(10), }}>
                        <Text style={customStyles.title}>{'khai báo y tế\n tại nhà'.toLocaleUpperCase()}</Text>
                    </View>
                    ...
                    <ListCauHoi ref={refTTCauHoi} type={2} />
                    {/* {renderChossImage()} */}
...
export default ToKhaiYTeTaiNha
