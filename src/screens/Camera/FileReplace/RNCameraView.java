...
public class RNCameraView extends CameraView implements LifecycleEventListener, BarCodeScannerAsyncTaskDelegate, FaceDetectorAsyncTaskDelegate,
  ...
  private byte[] byteframeDPS;
  private int widthframeDPS = 0;
  private int heightframeDPS = 0;
    ...
    addCallback(new Callback() {
      ...
      public void onFramePreview(CameraView cameraView, byte[] data, int width, int height, int rotation) {
        ...
        if (willCallFaceTask) {
          ...
          byteframeDPS = rotateNV21_working(data, width, height, correctRotation);
          widthframeDPS = width;
          heightframeDPS = height;
  ...
  public void onFacesDetected(WritableArray data) {
    int hsTop = (widthframeDPS - heightframeDPS)/2 - 20;
    int hsLeft = 0;
    int newWidth = heightframeDPS;
    int newHeight = heightframeDPS + hsTop;
    ByteArrayOutputStream out = new ByteArrayOutputStream();
    YuvImage yuvImage = new YuvImage(byteframeDPS, ImageFormat.NV21, heightframeDPS, widthframeDPS, null);
    yuvImage.compressToJpeg(new Rect(hsLeft, hsTop,
            newWidth, newHeight), 45, out);
    byte[] imageBytes = out.toByteArray();
    String frameDPSbase64 = Base64.encodeToString(imageBytes, Base64.DEFAULT);
    if (!mShouldDetectFaces) {
      return;
    }

    RNCameraViewHelper.emitFacesDetectedEvent(this, data, frameDPSbase64);
  }
