...
const HandlerLst = [
    {
        key: 1, name: 'Không duyệt', isLyDo: true, rule: 10057,
    },
    {
        key: 2, name: 'Duyệt', isLyDo: true, rule: 10057,
    },
    {
        key: 3, name: 'Xoá', isLyDo: false, rule: 10059,
    },
]
...
class ChiTietTuyenDung extends Component {
    ...
    _getHandler = () => {
        ...
        let arrHandler = []
        HandlerLst.forEach(item => {
            if (Rules.includes(item?.rule) && dataTuyenDung?.IsDuyet == 0 && [1, 2].includes(item.key)) {
                arrHandler.push(item)
            } else {
                if (Rules.includes(item?.rule) && ![1, 2].includes(item.key)) {
                    arrHandler.push(item)
                }
            }
        });
        this.setState({ dataHandler: arrHandler, selectedHandler: arrHandler.length > 0 ? arrHandler[0] : '' });
    }
    ...
    _getDetailTuyenDung = async () => {
        ...
        if (res == -1 || res == -3) {
            ...
            if (res.status == 1 && res.data) {
                ...
                if (key == 100 || key == 6) {
                    this.setState({ tuongtac: true, dataTuyenDung: res.data, isLoading: false }, this._getHandler);
                } else {
                    this.setState({ dataTuyenDung: res.data, isLoading: false }, this._getHandler);
                };
            } else {
                this.setState({ isLoading: false });
    ...
    onHanler = async () => {
        const { selectedHandler, NoiDung, dataTuyenDung } = this.state
        if (!selectedHandler?.key) {
            Utils.showToastMsg('Thông báo', 'Vui lòng chọn hành động xử lý', icon_typeToast.warning, 2000, icon_typeToast.warning)
            return
        }
        if ([1, 2].includes(selectedHandler?.key)) {
            if (!NoiDung) {
                Utils.showToastMsg('Thông báo', `Vui lòng nhập nội dung ${selectedHandler.name.toLowerCase()}`, icon_typeToast.warning, 2000, icon_typeToast.warning)
                return
            }
            Utils.showMsgBoxYesNo(this, 'Thông báo',
                `Bạn có chắc muốn ${selectedHandler.name.toLowerCase()} tin tuyển dụng này?`,
                `${selectedHandler.name}`, 'Xem lại', async () => {
                    const bodyDuyet = {
                        "IsDuyet": selectedHandler?.key, // 1 là không duyet, 2 là duyet
                        "LyDo": NoiDung,
                        "Id": dataTuyenDung?.Id
                    }
                    console.log('body duyet', bodyDuyet)
                    let res = await apis.ApiSanLamViec.Duyet_TinTucCongDan(bodyDuyet)
                    console.log('res duyet', res)
                    if (res.status == 1) {
                        Utils.showToastMsg('Thông báo', `Thực hiện thành công`, icon_typeToast.success, 2000, icon_typeToast.success)
                        ROOTGlobal.dataGlobal._handleUIKiemDuyet({ ...this.state.dataTuyenDung, IsDuyet: selectedHandler.key })
                        Utils.goback(this)
                    } else {
                        Utils.showToastMsg('Thông báo', `Thực hiện thất bại`, icon_typeToast.info, 2000, icon_typeToast.info)
                    }
                }
            )
            ...
            Utils.showMsgBoxYesNo(this, 'Thông báo',
                `Bạn có chắc muốn ${selectedHandler.name.toLowerCase()} tin tuyển dụng này?`,
                `${selectedHandler.name}`, 'Xem lại', async () => {
                    let res = await apis.ApiSanLamViec.Delete_TinTucCongDan(dataTuyenDung?.Id)
                    if (res.status == 1) {
                        Utils.showToastMsg('Thông báo', `${selectedHandler.name} thành công`, icon_typeToast.success, 2000, icon_typeToast.success)
                        ROOTGlobal.dataGlobal._handleUIKiemDuyet({ ...this.state.dataTuyenDung, IsDuyet: selectedHandler.key }, true)
                        Utils.goback(this)
                    } else {
                        Utils.showToastMsg('Thông báo', `${selectedHandler.name} thất bại`, icon_typeToast.info, 2000, icon_typeToast.info)
                    }
                }
            )
        }
    ...
    render() {
        const { userCD, userDH } = this.props.auth
        const { tuongtac, isLoading, message, dataTuyenDung, dataHandler, selectedHandler } = this.state;
                    ...
                    </View>
                    {
                        data && data?.Id ?
                            <>
                                ...
                                <View style={{ borderWidth: 0.5, borderColor: '#5FBCBE', margin: 10, padding: 10, borderRadius: 5 }}>
                                    <Text style={{ paddingLeft: 5, fontWeight: 'bold', marginBottom: 10 }}>{'Xử lý tin tuyển dụng'}</Text>
                                    <View style={{ flexDirection: 'row', paddingHorizontal: 10 }}>
                                        {dataHandler.map((item, index) => {
                                            return (
                                                <View key={index} style={{ borderRadius: 5, borderWidth: 0.5, borderColor: item.key == selectedHandler?.key ? '#5FBCBE' : colors.black_60, alignItems: 'center', marginLeft: index > 0 ? 5 : 0 }}>
                                                    <TouchableOpacity key={index} onPress={() => this.setState({ selectedHandler: item })}
                                                        style={{ padding: 10, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                                        <Image source={item.key == selectedHandler?.key ? Images.icCheckboxActive : Images.icCheckboxUnActive} resizeMode='contain' style={nstyles.nstyles.nIcon16} />
                                                        <Text style={{ color: item.key == selectedHandler?.key ? '#5FBCBE' : colors.black_60, paddingLeft: 5 }}>{item?.name}</Text>
                                                    </TouchableOpacity>
                                                </View>
                                            )
                                        })}
                                ...
                                {
                                    [1, 2].includes(selectedHandler.key) && <TextInput
                                        placeholder={`Nhập nội dung ${selectedHandler?.name?.toLowerCase()} ...`}
                                        multiline={true}
                                        onChangeText={text => this.setState({ NoiDung: text })}
                                        style={{ margin: 10, padding: 10, backgroundColor: colors.BackgroundHome, borderRadius: 5, minHeight: 100, textAlignVertical: 'top' }}
                                    />
                                }
                                <TouchableOpacity onPress={this.onHanler} activeOpacity={0.5} style={{ padding: 10, backgroundColor: '#0081B2', alignItems: 'center', margin: 10, borderRadius: 5 }}>
                                    <Text style={{ color: colors.white, fontWeight: 'bold' }}>{selectedHandler?.key ? 'Xử lý' : 'Chọn hành động xử lý'}</Text>
                                </TouchableOpacity>
                            </> : null
                    }
                    {/* <View style={{ height: 5, backgroundColor: colors.BackgroundHome }} /> */}
