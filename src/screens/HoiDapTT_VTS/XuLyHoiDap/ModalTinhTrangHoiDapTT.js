...
export default class ModalTinhTrangHoiDapTT extends Component {
    constructor(props) {
        super(props);
    ...
    _renderItem = (item) => {
        return <Fragment key={item.Id}>
            <TouchableOpacity onPress={this._select(item)} style={{ padding: 22, paddingVertical: 20 }}>
                <Text style={{ fontSize: sizes.reSize(16), color: (this.item.Id == item.Id ? colors.peacockBlue : 'black') }}>{item.TenTrangThai}</Text>
            </TouchableOpacity>
