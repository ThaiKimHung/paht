...
class GuiCanhBaoCovid extends Component {
    ...
    _sendSOS = () => {
        ...
        if (warning.length > 0) {
            ...
            Utils.showMsgBoxYesNo(this, 'Xác nhận yêu cầu',
                `Chức năng này chỉ phục vụ cho việc cảnh bảo covid và thông tin phải đảm bảo chính xác. 
        ...
        `, 'Gửi cảnh báo', 'Xem lại', () => this.guiCanhBaoCovid()
            )
        }
    ...
    guiCanhBaoCovid = async () => {
        ...
        if (res.status == 1) {
            Utils.showMsgBoxOK(this, 'Thông báo', 'Gửi cảnh báo covid thành công!', 'Xác nhận', () => {
                Utils.goscreen(this, 'ManHinh_Home')
            })
        } else {
            Utils.showMsgBoxOK(this, 'Thông báo', res.error?.message ? res.error.message : 'Gửi cảnh báo covid thất bại!', 'Xác nhận')
        }
    }
    ...
    render() {
        ...
        return (
            <View style={{ flex: 1, backgroundColor: colors.BackgroundHome }}>
                <HeaderCus
                    title={UtilsApp.getScreenTitle("ManHinh_CanhBaoCovid", 'Cảnh báo Covid')}
