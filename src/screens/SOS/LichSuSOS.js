...
class LichSuSOS extends Component {
    constructor(props) {
        super(props);
        this.state = {
            dataTinhTrang: [],
    ...
    _getListNameStatus = async () => {
        let res = await ApisAdmin.ApiSOS.GetList_TinhTrangAll()
    ...
    _GetListSOS = async () => {
        ...
        let obj = {
            ...objFilter,
            "keyword": keyword,
            "filter.vals": `${dateFrom ? moment(dateTo, 'YYYY-MM-DD').format('DD-MM-YYYY') : ''}|${dateFrom ? moment(dateFrom, 'YYYY-MM-DD').format('DD-MM-YYYY') : ''}|${tinhtrang.Id}|${DevicesToken}`,
            "page": page.Page,
        }
        let res = await apis.ApiUser.GetListSOSApp(obj)
    ...
    _goDetail = (item) => {
        Utils.goscreen(this, 'scChiTietSOS', { ID: item.Id, callback: () => Utils.goscreen(this, 'ManHinh_SOS') })
    }
