...
class FormTuVanF0 extends Component {
    constructor(props) {
        super(props);
        this.tieuDeGui = '';
        ...
        this.state = {
            ...
            PhoneNoLogin,
            dataTypeForm: '',
            NguonPA: 30, // 30 : lương thực, 31 y tế,
            isLoading: false,
            selectedHuyen: '',
            selectedXa: '',
            LstHuyen: [],
    ...
    componentDidMount() {
        ...
        this.getFirstStore();
        ROOTGlobal.dataGlobal._onListeningDataTuVanF0 = (data) => this.onListeningDataPA(data);
    ...
    handleTypeForm = async () => {
        ...
        switch (typeForm) {
            case 103:
                dataType = {
                    Warning: 'Vui lòng nhập nội dung yêu cầu tư vấn F0',
                    BodyPost: [
                        {
                            Name: '',
                            Key: 'TypeReference',
                    ...
                    ActionsSend: () => Utils.showMsgBoxOK(this, 'Thông báo', 'Gửi yêu cầu tư vấn F0 thành công', 'OK', () => Utils.goscreen(this, 'tab_TuVanF0CaNhan')),
                    PlaceholderNoiDung: 'Nhập nội dung yêu cầu tư vấn',
                    TextLocation: 'Địa chỉ',
                    HeaderTitle: 'Yêu cầu tư vấn F0',
                    KeyRenderMore: 103,
                    LabelNoiDung: `Nội dung yêu cầu tư vấn`
                }
                break;
            case 1000:
                dataType = {
        ...
        if ([-1, 103].includes(dataType?.KeyRenderMore)) {
    ...
    onRealTime = async () => {
        ...
        await Utils.nsetStore(nkey.realTimeDataTuVanF0, data)
    }

    onClearRealTime = async () => {
        await Utils.nsetStore(nkey.realTimeDataTuVanF0, {})
    }
    ...
    handleRemoveDraft = async () => {
        ...
        let draftListTuVanF0 = await Utils.ngetStore(nkey.draftListTuVanF0, []);
        let draftIndex = draftListTuVanF0.findIndex(item => item.idDraft == idDraft)
        if (draftIndex != -1) {
            draftListTuVanF0.splice(draftIndex, 1);
        }
        await Utils.nsetStore(nkey.draftListTuVanF0, draftListTuVanF0);
        ROOTGlobal.dataGlobal._onRefreshBanNhapTuVanF0();
        Utils.goback(this);
    ...
    handleUpdateDraft = async () => {
        ...
        if (draftIndex != -1) {
            ...
            draftListTuVanF0[draftIndex] = itemUpdate;
        }
        await Utils.nsetStore(nkey.draftListTuVanF0, draftListTuVanF0);
        ROOTGlobal.dataGlobal._onRefreshBanNhapTuVanF0();
        Utils.goback(this);
    ...
    onPressBack = () => {
        ...
        if (idDraft > 0) {
            Alert.alert(
                'Chỉnh sửa nháp',
                'Bạn đang thực hiện chỉnh sửa bản nháp yêu cầu hỗ trợ an sinh xã hội. ' +
            ...
            Alert.alert(
                'Lưu nháp',
                'Nội dung yêu cầu hỗ trợ an sinh sẽ mất sau khi bạn thoát. ' +
        ...
        Utils.showMsgBoxYesNo(this,
            ...
            'Đăng nhập', 'Quay lại', () => {
                ...
                if (this.props.auth.tokenCD && this.props.auth?.tokenCD?.length > 0) {
                    ...
                    Utils.goscreen(this, 'login', { isSendTuVan: true });
                }
            });
    }
    ...
    onPressSend = async () => {
        ...
        let {
            ...
            PhoneNoLogin,
            NameNoLogin,
            dataTypeForm, selectedXa, selectedHuyen
        } = this.state;
        diaDiem = diaDiem ? diaDiem : Utils.getGlobal(nGlobalKeys.isDiaChi, '') && ![103].includes(dataTypeForm.KeyRenderMore) ? appConfig.TenTinh : '';

        if ([-1, 103].includes(dataTypeForm?.KeyRenderMore)) {
            const { tokenCD } = this.props.auth
            if (!tokenCD) {
                this.goToLogin();
                return
        ...
        if (!selectedHuyen) {
            Toast.show('Vui lòng chọn huyện', Toast.LONG);
            return;
        }
        if (!selectedXa) {
        ...
        if (IdPA > 0) {
            ...
            dataBoDy.append("IdBoPhan", selectedXa?.MaDV)
    ...
    onSpeechError = async (e) => { // sự kiện khi không thu âm được giọng nói
        ...
        if (e.error.message === '9/Insufficient permissions' && result === 'granted') { // Trường hợp quyền mircophone bị tắt dưới app Google
            Utils.showMsgBoxYesNo(this, 'Thông báo', 'Bạn cần cấp quyền nhận dạng giọng nói Google để sử dụng chức năng này.', 'Đến cài đặt', 'Quay lại', () => { OpenSetting.openSetting('com.google.android.googlequicksearchbox') });
        }
    }
    ...
    onSpeechResultsHandler = (e) => { // sự kiện lắng nghe khi ngừng nói và xuất ra dữ liệu
        ...
        if (Platform.OS == 'ios') {
            if (this.oldContent.length != 0) {
                this.setState({ noiDungGui: this.oldContent + ' ' + this.newContent.toLocaleLowerCase() })
            } else {
            ...
            if (this.oldContent === '') {
                ...
                this.setState({ noiDungGui: this.oldContent + ' ' + this.newContent.toLowerCase() });
        }
    }
    ...
    GetHuyen = async () => {
        let res = await apis.ApiPhanAnh.List_HuyenThiXaThanhPho()
        Utils.nlog('res huyen', res)
        if (res.status == 1 && res.data) {
            this.setState({ LstHuyen: res.data })
        } else {
            this.setState({ LstHuyen: [] })
        }
    }

    GetPhuong = async () => {
        let res = await apis.ApiPhanAnh.GetDonVi_Id(this.state.selectedHuyen?.MaDonVi)
        Utils.nlog('res phuong', res)
        if (res.status == 1 && res.data) {
            this.setState({ LstXa: res.data, selectedXa: '' })
        } else {
            this.setState({ LstXa: [], selectedXa: '' })
        }
    }
    ...
    choosePhuongXa = () => {
        Utils.goscreen(this, 'Modal_ComponentSelectBottom', {
            callback: (val) => this.setState({ selectedXa: val }),
            item: this.state.selectedXa || '',
            title: 'Danh sách',
            AllThaoTac: this.state.LstXa || [],
            ViewItem: (item) => this._viewItem(item, 'TenDV'), Search: true, key: 'TenDV'
        })
    }

    chooseHuyen = () => {
        Utils.goscreen(this, 'Modal_ComponentSelectBottom', {
            callback: (val) => this.setState({ selectedHuyen: val }, this.GetPhuong),
            item: this.state.selectedHuyen || '',
            title: 'Danh sách',
            AllThaoTac: this.state.LstHuyen || [],
            ViewItem: (item) => this._viewItem(item, 'TenDonVi'), Search: true, key: 'TenDonVi'
        })
    }
    ...
    render() {
        let {
            ...
            NameNoLogin, PhoneNoLogin, dataTypeForm, NguonPA, isLoading,
                    ...
                    <ScrollView
                        ...
                        <View style={[nstyles.nbody]}>
                            <View style={[nstyles.nbody, { paddingHorizontal: 13 }]}>
                                <View style={nstyles.nbody}>
                                    {/* {Thành phần hiển thị khi gửi không có login} */}
                                    {
                                        this.sendOpinionNoLogin ?
                                            tokenCD || [-1, 103].includes(dataTypeForm?.KeyRenderMore) ? null :
                                                <>
                                    ...
                                    <Text style={{ marginTop: 10, fontWeight: 'bold' }}>{'Chọn Huyện'}<Text style={{ color: colors.redStar }}>*</Text></Text>
                                    <TouchableOpacity onPress={this.chooseHuyen} activeOpacity={0.5} style={{ padding: 10, backgroundColor: colors.BackgroundHome, borderRadius: 5, marginTop: 10 }}>
                                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                                            <Text style={{ flex: 1, paddingRight: 5, fontSize: reText(14) }} numberOfLines={1}>{selectedHuyen ? selectedHuyen?.TenDonVi : '- Chọn Huyện -'}</Text>
                                            <Image source={Images.icDropDown} resizeMode='contain' style={nstyles.nIcon14} />
                                        </View>
                                    </TouchableOpacity>
                                    <Text style={{ marginTop: 10, fontWeight: 'bold' }}>{'Chọn phường/xã'}<Text style={{ color: colors.redStar }}>*</Text></Text>
                                    <TouchableOpacity onPress={this.choosePhuongXa} activeOpacity={0.5} style={{ padding: 10, backgroundColor: colors.BackgroundHome, borderRadius: 5, marginTop: 10 }}>
                                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                                            <Text style={{ flex: 1, paddingRight: 5, fontSize: reText(14) }} numberOfLines={1}>{selectedXa ? selectedXa?.TenDV : '- Chọn Phường/Xã -'}</Text>
                                            <Image source={Images.icDropDown} resizeMode='contain' style={nstyles.nIcon14} />
                                        </View>
                                    </TouchableOpacity>
                                ...
                                {[1000, 103].includes(dataTypeForm?.KeyRenderMore) ? null :
                                    <>
                                        <TouchableOpacity
                                            activeOpacity={0.5}
                                            ...
                                            <View style={stViewTuDong}>
                                                ...
                                                <Text style={{ color: colors.black_50, fontSize: reText(16), marginLeft: 5 }}>
                                                    {'Yêu cầu hỗ trợ an sinh xã hội'} <Text style={{ color: colors.redStar }}>KHẨN</Text>
                                                </Text>
                                            </View>
                                        </TouchableOpacity>
...
export default Utils.connectRedux(FormTuVanF0, mapStateToProps, true)
