...
class DaGui extends Component {
    ...
    componentDidMount() {
        this.getFirstData();
        ROOTGlobal.dataGlobal._onRefreshDaGui = this._onRefresh;
    }
    getFirstData = async () => {
        await this.getListDaGui();
    }
    ...
    getListDaGui = async () => {
        ...
        let res = await apis.ApiPhanAnh.GetDanhSachPAFilterDaGui(_page, 10, this.props.objectfilter);
        // this.refLoading.current.hide();
        Utils.nlog("DS DaGui:", res);
    ...
    onEdit = async (item) => {
        ...
        console.log('LOG item phan anh', item)
        let tmpHinhAnh = [];
    ...
    _renderItem = (item, index) => {
        ...
        return (
            <ItemDanhSach
                ...
                isShare={false}
                goscreen={() => Utils.goscreen(this.nthis, 'Modal_ChiTietPhanAnh', { IdPA: IdPA, TenChuyenMuc: ChuyenMuc })}
    ...
    loadMore = () => {
        ...
        if (_page < _allPage) {
            this.setState({ _page: _page + 1 }, this.getListDaGui);
        }
    };
...
export default Utils.connectRedux(DaGui, mapStateToProps, true)
