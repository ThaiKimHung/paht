...
import { deleteKeyTimKiemThueNha, saveKeyTimKiemThueNha } from '../../../../srcRedux/actions/widgets'
import TextApp from '../../../../components/TextApp'
import { Images } from '../../../images'
import { colorsWidget, colors } from '../../../../styles/color'

const SearchThueNha = (props) => {
  const dispatch = useDispatch();
  const { LstTimKiemGanDayThueNha = [] } = useSelector(state => state.Widgets)
  ...
  const onSearch = () => {
    if (textSearch) {
      Utils.goback({ props })
      dispatch(saveKeyTimKiemThueNha(textSearch))
      callback(textSearch)
    } else {
  ...
  const onDeleteSearch = (item) => {
    dispatch(deleteKeyTimKiemThueNha(item))
  }
  ...
  return (
    <View style={stSearchThueNha.container}>
      <View style={stSearchThueNha.viewSearch}>
...
const stSearchThueNha = StyleSheet.create({
  container: {
    flex: 1
  },
  body: {
